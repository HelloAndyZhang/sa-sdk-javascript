sensorsDataAnalytic201505.modules['AesEncryption'] = (function () {
  ...
  function encryptData(originalData, key, iv, isRawKey) {
    ...
    var result = opt.iv
      .clone()
      .concat(CryptoJS.enc.Base64.parse(encryptedB64Val))
  ...
  var AesEncryption = {
    init: function (sd, config) {
      ...
      if (
        isTruthy(config, '初始配置config') &&
        isTruthy(config.k, '初始参数k') &&
        isTruthy(config.kid, '初始参数kid') &&
        isTruthy(config.khash, '初始参数khash') &&
        matchType(config.k, '初始参数k', 'string') &&
        matchType(config.khash, '初始参数khash', 'string') &&
        matchType(config.kid, '初始参数kid', 'number')
      ) {
        _config = config
        _iv = generateIVData()
  ...
  if (
    ...
    window.SensorsDataWebJSSDKPlugin.AesEncryption =
      window.SensorsDataWebJSSDKPlugin.AesEncryption || AesEncryption
  } else {
    window.SensorsDataWebJSSDKPlugin = {
