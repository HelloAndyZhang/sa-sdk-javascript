;(function (global, factory) {
          ...
          if (old_login_id_name && state.identities.hasOwnProperty(old_login_id_name)) {
            ...
            for (identitiesprop in state.identities) {
              if (state.identities.hasOwnProperty(identitiesprop)) {
                if (
                  identitiesprop !== '$identity_cookie_id' &&
                  identitiesprop !== currentLoginKey
                ) {
                  delete state.identities[identitiesprop]
                }
              }
  ...
  function loginBody(obj) {
    ...
    var isNewLoginId =
      sd.store._state.history_login_id.name !== name ||
      id !== sd.store._state.history_login_id.value
    if (isNewLoginId) {
                ...
                _localStorage.set(
                  ...
                  JSON.stringify(
                    self.generateTabStorageVal(
                      (
                        safeJSONParse(_localStorage.get(self.tabKey)) ||
  ...
  var bridge = {
    ...
    initPara: function () {
      var app_js_bridge_default = {
        is_send:
          sd.para.use_app_track_is_send === false || sd.para.use_app_track === 'only'
            ? false
            : true,
        white_list: [],
  ...
  SDKJSBridge.prototype.call = function (callArg, timeout) {
    var that = this
    var callId =
      new Date().getTime().toString(16) + String(getRandom()).replace('.', '').slice(1, 8)
          ...
          if (url_info) {
            ...
            if (isArray(confs) && confs.length > 0) {
              each(confs, function (propConf) {
                ...
                if (
                  propConf.url_host === url_info.host &&
                  propConf.url_path === url_info.pathname
                ) {
                  if (propConf.h5) {
        ...
        if (isObject(sd.para.app_js_bridge) && bridgeObj.verify_success === 'success') {
          if (!isLoaded) {
            ...
            loadScript({
              ...
              type: 'js',
              url:
                protocol + '//static.sensorsdata.cn/sdk/' + sd.lib_version + '/vapph5define.min.js'
            })
          } else {
            sa_jssdk_app_define_mode(sd, isLoaded)
          }
  ...
  try {
    ...
    sd.modules['AndroidBridge'] = (function () {
      ...
      function callBridge(request, bridge) {
        return (
          bridge && typeof bridge[request.callType] === 'function' && bridge[request.callType]()
        )
      }
