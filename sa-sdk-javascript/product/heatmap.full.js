  ...
  ;(function () {
    ...
    var heatmap_render = {
      ...
      getCurrentUrl: function () {
        ...
        obj['sa-request-url'] = obj['sa-request-url']
          ? encodeURIComponent(obj['sa-request-url'])
          : ''
        ...
        if (typeof id === 'string' && sd.para.web_url) {
          ...
          var suc = function (data) {
            ...
            function showLineDetail(e) {
              ...
              over_ele = $(
                over_tpl.replace('{{top}}', e.pageY + 15).replace('{{percent}}', i + '%')
              )
      ...
      setToolbar: function (requrest_id, type, url) {
        ...
        filterFlyout += '</div>'
        filterFlyout +=
          '<div style="height: 47px; overflow: hidden; padding: 10px 15px; box-sizing: border-box;">'
        filterFlyout +=
        ...
        $(document.body).on(
          'change',
          '#sa_sdk_heatmap_filterFlyout select[name="filter_type"]',
          function () {
            var type = $(this).val()
      ...
      setCssStyle: function () {
        ...
        css +=
          '#sa_heat_float_right_box_content table td { color: #fff !important; font-size: 13px !important;}'
        css +=
          ...
          _.jsonp({
            ...
            error: function (err) {
              ...
              if (err === 'timeout') {
                ...
                if (
                  _.URL(location.href).protocol === 'https:' &&
                  _.URL(obj.url.jsonp).protocol === 'http:'
                ) {
                  error({
                  ...
                  error({
                    error:
                      '<div>后台 JSONP 地址不通，可能原因如下：</div><div style="font-size:15px;margin-top:10px;text-align:left;display:inline-block"><div>1. 检查神策分析版本是否低于 v2.2.0，如果低于此版本请联系值班同学升级版本</div><div style="margin-top:5px">2. 神策分析后台不允许访问，请贵司研发参考当前页面的控制台报错，去对应的后台配置解决</div></div>'
                  })
                }
              }
            }
      ...
      prepare: function (data, type, url) {
        ...
        function hasGetWebUrl() {
          setTimeout(
            ...
            _.isObject(sd.para.heatmap) && sd.para.heatmap.loadTimeout
              ? sd.para.heatmap.loadTimeout
              : 2500
          )
        }
