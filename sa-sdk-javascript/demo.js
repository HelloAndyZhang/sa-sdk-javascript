            ...
            switch (charCode) {
              ...
              default:
                ...
                if (charCode >= 48 && charCode <= 57) {
                  if (
                    charCode == 48 &&
                    ((charCode = source.charCodeAt(Index + 1)), charCode >= 48 && charCode <= 57)
                  ) {
                    abort()
                  }
                  isSigned = false
...
function sdLog() {
  if (
    (_sessionStorage.isSupport() && sessionStorage.getItem('sensorsdata_jssdk_debug') === 'true') ||
    sdPara.show_log
  ) {
    if (
      isObject(arguments[0]) &&
      (sdPara.show_log === true || sdPara.show_log === 'string' || sdPara.show_log === false)
    ) {
...
var pageInfo = {
  ...
  properties: function () {
    var viewportHeightValue =
      window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight || 0
    var viewportWidthValue =
...
function getInputElementValue(inputEle) {
  var allowCollectInputVal =
    sdPara.heatmap &&
    typeof sdPara.heatmap.collect_input === 'function' &&
    sdPara.heatmap.collect_input(inputEle)
...
function addEvent$1(target, eventName, evenHandler) {
  ...
  if (
    ...
    typeof sdPara.heatmap.useCapture === 'undefined' &&
    eventName === 'click'
  ) {
    useCapture = true
  }
...
function isNewUser() {
  ...
  if (cookie$1.isSupport()) {
    if (
      cookie$1.get('sensorsdata_is_new_user') !== null ||
      cookie$1.get(getNewUserFlagKey(prefix)) !== null
    ) {
      return true
    }
    return false
...
var store = {
  ...
  getUnionId: function (state) {
    ...
    if (
      ...
      this._state.history_login_id.name !== sd.IDENTITY_KEY.LOGIN
    ) {
...
var heatmap = {
  ...
  getTargetElement: function (element, e) {
    ...
    if (
      ...
      ry(parent_ele).prev().tagName &&
      ry(parent_ele).prev().tagName.toLowerCase() === 'img'
    ) {
  ...
  setNotice: function (web_url) {
    ...
    if (!sd.para.heatmap_url) {
      sd.para.heatmap_url =
        getSafeHttpProtocol() + '//static.sensorsdata.cn/sdk/' + sd.lib_version + '/heatmap.min.js'
    }
  },
  ...
  start: function (ev, target, tagName, customProps, callback) {
    ...
    var userCallback = isFunction(callback)
      ? callback
      : isFunction(customProps)
      ? customProps
      : undefined
    if (
      sd.para.heatmap &&
  ...
  isStyleTag: function (tagname, isVisualMode) {
    ...
    if (
      isVisualMode &&
      (!sd.para.heatmap || !sd.para.heatmap.collect_tags || !sd.para.heatmap.collect_tags.div)
    ) {
      ...
      indexOf(sd.para.heatmap.collect_tags.div.ignore_tags, tagname) > -1
    ) {
      return true
    }
    return false
  ...
  trackHeatMap: function (target, props, callback) {
    if (typeof target === 'object' && target.tagName) {
      ...
      if (
        ...
        tagName !== 'input' &&
        tagName !== 'textarea' &&
        !hasAttributes(target, trackAttrs)
      ) {
        ...
        extend(
          {
            ...
            $first_browser_charset:
              typeof document.charset === 'string' ? document.charset.toUpperCase() : '取值异常',
...
function initPara(para) {
  ...
  sd.para.preset_properties = extend(
    {},
  ...
  if (
    ...
    (sd.para.web_url.slice(0, 3) === '://' || sd.para.web_url.slice(0, 2) === '//')
  ) {
  ...
  if (typeof sd.para.source_type === 'object') {
    ...
    sd.para.source_type.keyword = isObject(sd.para.source_type.keyword)
      ? extend(search_keyword, sd.para.source_type.keyword)
      : search_keyword
  }
...
function use(name, option) {
  ...
  if (
    ...
    isObject(window.SensorsDataWebJSSDKPlugin[name]) &&
    isFunction(window.SensorsDataWebJSSDKPlugin[name].init)
  ) {
    window.SensorsDataWebJSSDKPlugin[name].init(sd, option)
    return window.SensorsDataWebJSSDKPlugin[name]
  } else if (
    isObject(sd.modules) &&
    isObject(sd.modules[name]) &&
    isFunction(sd.modules[name].init)
  ) {
...
function unbind(itemName, itemValue) {
  ...
  if (
    isObject(sd.store._state.identities) &&
    sd.store._state.identities.hasOwnProperty(itemName) &&
    sd.store._state.identities[itemName] === itemValue
  ) {
...
function loginBody(obj) {
  ...
  if (
    isObject(sd.store._state.identities) &&
    sd.store._state.identities.hasOwnProperty(name) &&
    id === sd.store._state.first_id
  ) {
    return false
  }

  var isNewLoginId =
    sd.store._state.history_login_id.name !== name || id !== sd.store._state.history_login_id.value
  if (isNewLoginId) {
...
kit.sendData = function (data, callback) {
  console.log('Kit.sendData', data.event, data)
              ...
              _localStorage.set(
                ...
                JSON.stringify(
                  self.generateTabStorageVal(
                    (
                      safeJSONParse(_localStorage.get(self.tabKey)) || this.generateTabStorageVal()
...
var bridge = {
  ...
  initPara: function () {
    var app_js_bridge_default = {
      is_send:
        sd.para.use_app_track_is_send === false || sd.para.use_app_track === 'only' ? false : true,
      white_list: [],
  ...
  hasVisualModeBridge: function () {
    ...
    return (
      isObject(vBridgeObj) &&
      vBridgeObj[vMode] &&
      (vBridgeObj[vMode] === true || vBridgeObj[vMode]())
    )
  },
...
vtrackBase.filterConfig = function (data, events, page_url) {
  ...
  if (data.event === '$WebClick') {
    each(events, function (item) {
      if (
        isObject(item) &&
        (item.event_type === 'webclick' || item.event_type === 'appclick') &&
        isObject(item.event)
      ) {
...
vtrackBase.getAssignConfigs = function (func, config) {
  ...
  each(config.events, function (event) {
    if (
      ...
      event.event.url_host === url_info.page_url.host &&
      event.event.url_path === url_info.page_url.pathname
    ) {
      if (func(event)) {
        arr.push(event)
      }
...
var unlimitedDiv = {
  ...
  isTargetEle: function (ele) {
    ...
    for (var i = 0; i < this.events.length; i++) {
      if (
        isObject(this.events[i]) &&
        isObject(this.events[i].event) &&
...
var vapph5CustomProp = {
  ...
  geth5Props: function (data) {
    ...
    if (props.sensorsdata_app_visual_properties) {
      props.sensorsdata_app_visual_properties = base64Encode(
        JSON.stringify(props.sensorsdata_app_visual_properties)
      )
    }
...
var heatmapMode = {
  ...
  isWindowNameHasKeyword: function () {
    try {
      var nameParams = JSON.parse(window.name)
      var page_url = isString(nameParams['sa-request-page-url'])
        ? _decodeURIComponent(nameParams['sa-request-page-url'])
        : null
      return (
        nameParams['sa-request-id'] &&
        isString(nameParams['sa-request-id']) &&
  ...
  loadVtrack: function () {
    loadScript({
      ...
      url: sd.para.vtrack_url
        ? sd.para.vtrack_url
        : getSafeHttpProtocol() + '//static.sensorsdata.cn/sdk/' + sd.lib_version + '/vtrack.min.js'
    })
  },
...
function registerFeature(feature) {
  ...
  feature &&
    feature.businessStage &&
    businessStage.registerStageImplementation(feature.businessStage)
...
try {
  ...
  sd.modules['Amp'] = (function () {
    ...
    if (
      ...
      Object.prototype.toString.call(window.SensorsDataWebJSSDKPlugin) === '[object Object]'
    ) {
  ...
  sd.modules['AndroidBridge'] = (function () {
    ...
    function initBridge() {
      ...
      anServerUrl =
        anTrack && anBridge.sensorsdata_get_server_url && anBridge.sensorsdata_get_server_url()
    ...
    if (
      ...
      window.SensorsDataWebJSSDKPlugin.AndroidBridge =
        window.SensorsDataWebJSSDKPlugin.AndroidBridge || AndroidBridge
    } else {
      window.SensorsDataWebJSSDKPlugin = {
  ...
  sd.modules['AndroidObsoleteBridge'] = (function () {
    ...
    if (
      ...
      window.SensorsDataWebJSSDKPlugin.AndroidObsoleteBridge =
        window.SensorsDataWebJSSDKPlugin.AndroidObsoleteBridge || AndroidObsoleteBridge
    } else {
      window.SensorsDataWebJSSDKPlugin = {
  ...
  sd.modules['ChannelUtm'] = (function () {
    ...
    if (
      ...
      window.SensorsDataWebJSSDKPlugin.ChannelUtm =
        window.SensorsDataWebJSSDKPlugin.ChannelUtm || ChannelUtm
    } else {
      window.SensorsDataWebJSSDKPlugin = {
  ...
  sd.modules['Channel'] = (function () {
    ...
    if (
      ...
      window.SensorsDataWebJSSDKPlugin.SensorsChannel =
        window.SensorsDataWebJSSDKPlugin.SensorsChannel || Channel
    } else {
      window.SensorsDataWebJSSDKPlugin = {
  ...
  sd.modules['Deeplink'] = (function () {
    ...
    var handleAndroidLinks = function handleAndroidLinks(dp, scheme, downloadURL) {
    ...
    var SADeepLink = {
      ...
      init: function init() {
        ...
        this.apiURL = this.apiURL
          .replace('{origin}', serverInfo.origin)
          .replace('{project}', serverInfo.project)
    ...
    if (!isObject(window.SensorsDataWebJSSDKPlugin)) {
      ...
      window.SensorsDataWebJSSDKPlugin.Deeplink =
        window.SensorsDataWebJSSDKPlugin.Deeplink || SADeepLink
      window.SensorsDataWebJSSDKPlugin.deeplink =
        window.SensorsDataWebJSSDKPlugin.deeplink || SADeepLink
    }
  ...
  sd.modules['IosBridge'] = (function () {
    ...
    function handleCommand(request) {
      var callType = request.callType
      if (
        (callType === 'page_info' || callType === 'visualized_track') &&
      ...
      if (callType === 'sensorsdata_get_app_visual_config') {
        return (
          ...
          window.SensorsData_APP_New_H5_Bridge[callType]
        )
      }
    ...
    if (
      ...
      window.SensorsDataWebJSSDKPlugin.IOSBridge =
        window.SensorsDataWebJSSDKPlugin.IOSBridge || IOSBridge
    } else {
      window.SensorsDataWebJSSDKPlugin = {
        IOSBridge: IOSBridge
      }
  ...
  sd.modules['IosObsoleteBridge'] = (function () {
    ...
    if (
      ...
      window.SensorsDataWebJSSDKPlugin.IOSObsoleteBridge =
        window.SensorsDataWebJSSDKPlugin.IOSObsoleteBridge || IOSObsoleteBridge
    } else {
      window.SensorsDataWebJSSDKPlugin = {
  ...
  sd.modules['Pageleave'] = (function () {
      ...
      if (sd) {
        ...
        if (option) {
          ...
          if (
            ...
            (this._.isNumber(heartbeat_interval_time) ||
              this._.isNumber(heartbeat_interval_time * 1)) &&
    ...
    PageLeave.prototype.reissueHeartBeatData = function () {
      ...
      for (var i = storage_length - 1; i >= 0; i--) {
        ...
        if (
          ...
          if (
            this._.isObject(item_value) &&
            new Date() * 1 - item_value.time > item_value.heartbeat_interval_time + 5000
          ) {
    ...
    if (
      ...
      window.SensorsDataWebJSSDKPlugin.PageLeave =
        window.SensorsDataWebJSSDKPlugin.PageLeave || pageLeave
    } else {
      window.SensorsDataWebJSSDKPlugin = {
        PageLeave: pageLeave
      }
  ...
  sd.modules['Pageload'] = (function () {
    ...
    if (
      ...
      window.SensorsDataWebJSSDKPlugin.PageLoad =
        window.SensorsDataWebJSSDKPlugin.PageLoad || PageLoad
    } else {
      window.SensorsDataWebJSSDKPlugin = {
        PageLoad: PageLoad
      }
  ...
  sd.modules['RegisterProperties'] = (function () {
    ...
    function addProperties(data, instance) {
      ...
      _.each(currentProps, function (prop) {
        if (_.isObject(prop)) {
          ...
          if (
            _.isObject(callbackProp) &&
            !_.isEmptyObject(callbackProp) &&
            check({
    ...
    RegisterProperties.prototype.register = function (customProps) {
      ...
      if (
        this._.isObject(customProps) &&
        this._.isArray(customProps.events) &&
        customProps.events.length > 0 &&
        this._.isObject(customProps.properties) &&
        !this._.isEmptyObject(customProps.properties)
      ) {
    ...
    if (
      ...
      window.SensorsDataWebJSSDKPlugin.RegisterProperties =
        window.SensorsDataWebJSSDKPlugin.RegisterProperties || instance
    } else {
      window.SensorsDataWebJSSDKPlugin = {
  ...
  sd.modules['RegisterPropertyPageHeight'] = (function () {
    ...
    if (
      ...
      window.SensorsDataWebJSSDKPlugin.RegisterPropertyPageHeight =
        window.SensorsDataWebJSSDKPlugin.RegisterPropertyPageHeight || RegisterPropertyPageHeight
    } else {
      window.SensorsDataWebJSSDKPlugin = {
  ...
  ;(function () {
    ...
    siteLinker.getCurrenId = function () {
      ...
      if (this._.urlSafeBase64 && this._.urlSafeBase64.encode) {
        distinct_id = distinct_id
          ? this._.urlSafeBase64.trim(this._.urlSafeBase64.encode(this._.base64Encode(distinct_id)))
          : ''
      ...
      if (this._.isArray(sa_id) && sa_id[1]) {
        ...
        if (uid && (uid.substring(0, 1) === 'f' || uid.substring(0, 1) === 'd')) {
          if (
            ...
            this._.urlSafeBase64.isUrlSafeBase64(uid)
          ) {
            uid =
              uid.substring(0, 1) +
              this._.base64Decode(this._.urlSafeBase64.decode(uid.substring(1)))
    ...
    siteLinker.addListen = function () {
      ...
      var clickFn = function (event) {
        ...
        if (
          ...
          (parent_target &&
            parent_target.tagName &&
            parent_target.tagName.toLowerCase() === 'a' &&
            parent_target.href)
        ) {
      ...
      function resolveOption(option) {
        ...
        for (var i = 0; i < len; i++) {
          if (
            ...
            Object.prototype.toString.call(option[i].after_hash) == '[object Boolean]'
          ) {
            arr.push(option[i])
          } else {
    ...
    if (
      ...
      window.SensorsDataWebJSSDKPlugin.SiteLinker =
        window.SensorsDataWebJSSDKPlugin.SiteLinker || siteLinker
    } else {
      window.SensorsDataWebJSSDKPlugin = {
...
// sd$1.use('PageLeave')
// sd$1.use('PageLoad')
// sd$1.use('RegisterPropertyPageHeight')
export default sd$1
